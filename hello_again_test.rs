...
//! Test to put hello.txt with "hello again" text into the distributed storage cluster
//! Run with: rustc hello_again_test.rs && ./hello_again_test
...
fn main() {
    println!("📁 File Update Test: Uploading 'hello.txt' with 'hello again'");
    println!("==============================================================");
    ...
    // Test: Store/update file with "hello again" content
    println!("\n📋 Test: Storing file 'hello.txt' with content 'hello again'");
    
    let file_key = "hello.txt".to_string();
    let file_content = "hello again".as_bytes().to_vec();
    ...
    for node in &cluster {
        ...
        for (filename, content) in &node.storage {
            ...
            println!("  📄 {}: '{}' ({} bytes)", filename, text_content, content.len());
        }
    }

    println!("\n🎉 File upload with 'hello again' completed successfully!");
}